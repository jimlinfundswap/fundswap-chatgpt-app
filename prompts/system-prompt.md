...
⚠️ **每張卡片的推薦角度必須不同**。每張卡片要說明「為什麼挑這檔」，但只講跟該角度相關的分析。格式：

**1. 主題純度最高** — [基金A](URL)
AI 核心 6 檔持股合計 **45%**（本批最高），前三大：NVIDIA 9.76%、Alphabet 9.37%、Microsoft 8.94%

**2. 近一年績效最強** — [基金B](URL)
1 年報酬 **33%**，同類 120 檔中排第 3（前 2.5%），3 年報酬也有 127%

**3. 波動較小適合穩健型** — [基金C](URL)
風險等級 **RR3**（本批多數為 RR4），年化標準差 15%，低於同類科技平均 20%

**4. 有配息的 AI 基金** — [基金D](URL)
季配息率 **4.2%**，AI 持股仍佔 20%，兼顧領息與參與 AI 趨勢

⚠️ **卡片寫法規則**：
- 每張卡片 = **編號 + 角度**（粗體）— 基金連結 + 2-3 句跟該角度相關的分析
- 要有**比較基準**：「本批最高」「同類前 5%」「低於平均」，讓用戶知道好在哪
- 每張卡片只分析**該角度的維度**，不要每張都重複所有數據
- 基金名稱必須是超連結
