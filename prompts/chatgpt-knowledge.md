...
每張卡片角度不同，要說明「為什麼挑這檔」，但只講跟該角度相關的分析：

**1. 績效最強** — [基金A](URL)
1 年報酬 **25.3%**，同類 120 檔中排第 5（前 4%），近 3 個月仍有 12% 動能持續

**2. 穩穩領息** — [基金B](URL)
月配息率 **6.5%**，年化標準差僅 8.2%，波動小又有穩定現金流

**3. 主題純度最高** — [基金C](URL)
AI 持股合計 **51%**（本批最高），前三大：NVIDIA 9.7%、台積電 5.2%、Microsoft 4.8%

**4. CP 值最高** — [基金D](URL)
夏普指數 **1.8**（本批最高），每承受 1% 風險換到 1.8% 報酬

規則：每張卡片要有比較基準（本批最高、同類前 X%、低於平均），讓用戶知道好在哪。只分析該角度的維度
...
用推薦卡片格式，精選 3-5 檔，每張卡片角度不同，附分析說明：

**1. 主題純度最高** — [基金A](URL)
AI 持股合計 **51.6%**（本批最高），前三大：NVIDIA 9.76%、Alphabet 9.37%、Microsoft 8.94%

**2. 績效最強** — [基金B](URL)
1 年報酬 **33.2%**（本批最高），同類排前 5%，3 年報酬也有 85%

**3. 波動較小** — [基金C](URL)
風險等級 **RR3**（本批多數 RR4），年化標準差 15%，低於同類科技平均 20%

每張卡片要有比較基準，讓用戶知道「好在哪」
