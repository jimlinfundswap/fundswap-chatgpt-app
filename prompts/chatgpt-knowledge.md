...
⚠️ **策略 E（夏普效率）是預設必選**。除非用戶明確只問配息率或特定主題持股，否則每次推薦都應包含一張夏普/CP值角度的卡片。夏普指數幫用戶看「花同樣的風險能換到多少報酬」，是最實用但最常被忽略的指標。

| 用戶特徵 | 建議策略組合 |
|---------|-----------|
| 完全沒方向 | A + E + F（動能 + 效率 + 配息，三種差異大的方向） |
| 想積極投資 | A + B + E + K（短期動能 + 趨勢確認 + 夏普效率 + 主題集中） |
| 想穩健領息 | F + G + E（配息 + 低波動 + 夏普效率） |
| 想長期投資 | D + E + I（長期冠軍 + 夏普效率 + 定期定額） |
| 看好特定市場 | J + E + C（區域精選 + 夏普效率 + 年度績優） |
| 問特定主題（如 AI） | K + A + E（主題集中 + 動能 + 夏普效率） |
